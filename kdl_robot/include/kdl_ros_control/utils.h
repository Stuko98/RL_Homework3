...
inline Eigen::Matrix3d skew(const Eigen::Vector3d & t)                        //a partire da un vettore 3D, genera la matrice S(vettore) skew-symmetric di quel vettore. Tale matrice è tale che: il prodotto di S(vettore A) con un vettore B coincide con il prodotto vettoriale tra vettore A e vettore B. "Robotics" di Siciliano -> cap 3 -> 3.1.1 ->(3.9)                     
{
...
pseudoinverse(const MatT &mat, typename MatT::Scalar tolerance = typename MatT::Scalar{1e-4}) // choose appropriately              //calcola la pseudoinversa classica (quella di Moore-Penrose) 
{
...
inline Eigen::Matrix<double,3,1> computeOrientationError(const Eigen::Matrix<double,3,3> &_R_d,               //"Robotics" di Siciliano -> cap 3 -> 3.7.3 -> (3.85)
...
inline Eigen::Matrix<double,3,1> computeLinearError(const Eigen::Matrix<double,3,1> &_p_d,                   //"Robotics" di Siciliano -> cap 3 -> 3.7.3 -> (3.78)
                                                    const Eigen::Matrix<double,3,1> &_p_e)                   //lo usa anche per l'errore di velocità posizionale
{                                                                                                            //lo potremmo usare anche per l'errore di velocità di orientamento con angoli di Eulero
    return _p_d - _p_e;                                                                                      
}

inline Eigen::Matrix<double,3,1> computeOrientationVelocityError(const Eigen::Matrix<double,3,1> &_omega_d,  //"Robotics" di Siciliano -> cap 3 -> 3.7.3 -> (3.88)
...
inline Eigen::MatrixXd weightedPseudoInverse(const Eigen::MatrixXd &W,                                          //pseudoinversa con metodo alternativo
...
inline Eigen::Matrix<double,3,1> computeEulerAngles(const Eigen::Matrix<double,3,3> &_R)                                        //"Robotics" di Siciliano -> cap 2 -> 2.4.1 -> (2.19)    
{
    Eigen::Matrix<double,3,1> euler;
    //EULER ANGLE WITH ANGLE AXIS CONVENTION
    double r13=_R(0,2);
    double r23=_R(1,2);
    double r31=_R(2,0);
    double r32=_R(2,1);
...
inline Eigen::Matrix<double,3,1> computeOrientationErrorEuler(const Eigen::Matrix<double,3,3> &_R_d,                               //"Robotics" di Siciliano -> cap 3 -> 3.7.3 -> (3.80)
                                                         const Eigen::Matrix<double,3,3> &_R_e)
{
...
inline Eigen::Matrix<double,3,3> T_matrix(const Eigen::Matrix<double,3,1> &euler){                                                 //"Robotics" di Siciliano -> cap 3 -> 3.6 -> (3.64)
...
inline Eigen::Matrix<double,3,3> Tdot_matrix(const Eigen::Matrix<double,3,1> &euler){                                              //derivata rispetto a "euler"
    double phi = euler(0,0);
    double theta = euler(1,0);
    Eigen::Matrix<double,3,3> Tdot;
    /*{
        {0, -cos(phi),cos(phi)*cos(theta)-sin(phi)*sin(theta)},
        {0, -sin(phi),sin(phi)*cos(theta)+cos(phi)*sin(theta)},
        {0, 0, -sin(theta)}
    };*/
    Tdot(0,0) = 0;
    Tdot(0,1) = -cos(phi);
...
inline  Eigen::Matrix<double,6,7> AnalitycalJacobian(const Eigen::Matrix<double,6,7> &J,const Eigen::Matrix<double,3,1> &euler){   //"Robotics" di Siciliano -> cap 3 -> 3.6 -> (3.65)
